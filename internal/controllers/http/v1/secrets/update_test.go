...
func TestUpdate(t *testing.T) {
	...
	t.Run("password", func(t *testing.T) {
		...
		service.EXPECT().
			Update(gomock.Any(), testID, testOwnerID, testSecretKey, testName, &domain.PasswordData{
				Login:    testLogin,
	...
	t.Run("card", func(t *testing.T) {
		...
		service.EXPECT().
			Update(gomock.Any(), testID, testOwnerID, testSecretKey, testName, &domain.CardData{
				Number: testCard,
				Holder: testHolder,
				Exp:    testExp,
				CVV:    testCVV,
	...
	t.Run("text", func(t *testing.T) {
		...
		service.EXPECT().
			Update(gomock.Any(), testID, testOwnerID, testSecretKey, testName, &domain.TextData{
	...
	t.Run("file", func(t *testing.T) {
		...
		service.EXPECT().
			Update(gomock.Any(), testID, testOwnerID, testSecretKey, testName, &domain.FileData{
...
var emptyUpdateErrors = map[models.SecretType][]string{
	models.SecretTypePwd: {
...
func TestUpdate_EmptyBody_Fails_UnprocessableEntity(t *testing.T) {
	t.Parallel()
	root := gin.Default()
	secrets.AddRoutes(&root.RouterGroup, nil, guardMock)

	for _, secretType := range secretTypes {
		testName := secretType.String()
		t.Run(testName, func(t *testing.T) {
			t.Parallel()

			result := apitest.New(testName).
				Handler(root.Handler()).
				Debug().
				Putf("/secrets/%s/%s", secretType.String(), testID).
				Body("{}").
			...
			assert.ElementsMatch(t, emptyUpdateErrors[secretType], response.Errors)
		})
	}
}
